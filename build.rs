...
mod sinh_approx {
    ...
    fn generate_file_contents() -> String {
        let mut code = "//! Generated by `build.rs::sinh_approx::generate_file_contents()`
        #![allow(clippy::all)]
        #![allow(dead_code)]
    ...
    pub(super) fn run() {
        assert_eq!(
            PIECEWISE_VALUES.len(),
            PIECEWISE_CUTOFFS.len(),
            "PIECEWISE_CUTOFFS and PIECEWISE_VALUES must be the same length"
        );

        let code = generate_file_contents();
        ...
        if !output.status.success() {
            ...
            writeln!(lock, "===== RUSTFMT STDOUT =====")
                .expect("Failed to write rustfmt stdout header");
            lock.write_all(&output.stdout)
                .expect("Failed to write rustfmt stdout");
            drop(lock);

            let mut lock = stderr().lock();
            writeln!(lock, "===== RUSTFMT STDERR =====")
                .expect("Failed to write rustfmt stderr header");
            lock.write_all(&output.stderr)
                .expect("Failed to write rustfmt stderr");

            if let Some(c) = output.status.code() {
                panic!("Failed to run rustfmt: process exited with status code {c}",);
            } else {
                #[cfg(unix)]
                use std::os::unix::process::ExitStatusExt;
                #[cfg(unix)]
                if let Some(s) = output.status.signal() {
                    panic!("Failed to run rustfmt: process exited with signal {s}",);
                } else if let Some(s) = output.status.stopped_signal() {
                    panic!("Failed to run rustfmt: process exited with stop signal {s}",);
                }

                panic!("Failed to run rustfmt: unknown reason");
            }
        }
    }
}
