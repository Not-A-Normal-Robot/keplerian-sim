...
fn poll_vel(orbit: &impl OrbitTrait) {
    for i in 0..POLL_ANGLES {
        ...
        black_box(orbit.get_velocity_at_time(black_box(angle)));
    }
}
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("vel time poll cached", |b| {
        b.iter(|| poll_vel(black_box(&orbit)))
    });

    let compact: CompactOrbit = orbit.into();

    c.bench_function("vel time poll compact", |b| {
    ...
    c.bench_function("vel time poll hyp cached", |b| {
        b.iter(|| poll_vel(black_box(&orbit)))
    });

    let compact: CompactOrbit = orbit.into();

    c.bench_function("vel time poll hyp compact", |b| {
        b.iter(|| poll_vel(black_box(&compact)))
    });
}
