...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("sv poll cached", |b| b.iter(|| poll_sv(black_box(&orbit))));

    let compact: CompactOrbit = orbit.into();

    c.bench_function("sv poll compact", |b| {
    ...
    c.bench_function("sv poll hyp cached", |b| {
        b.iter(|| poll_sv(black_box(&orbit)))
    });

    let compact: CompactOrbit = orbit.into();

    c.bench_function("sv poll hyp compact", |b| {
        b.iter(|| poll_sv(black_box(&compact)))
    });
}
